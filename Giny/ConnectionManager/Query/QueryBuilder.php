...
class QueryBuilder
{
	...
	const PARAM_PREFIX = 'some_param_';

	protected $_paramCounter = 1;
	 ...
	 * @throws Exception
	 * @internal param array $query the query specification in name-value pairs. The following
	...
	public function buildQuery()
	{
		...
		if(!empty($query['from']))
			...
			throw new Exception(\Yii::t('yii','The DB query must contain the "from" portion.'));
		...
		return new Query($sql, $this->_params,$this->_values);
	}
	 ...
	 * @return QueryBuilder the command object itself
	 * @since 1.1.6
	 */
			...
			foreach($columns as $i=>$column)
			{
				...
				elseif(strpos($column,'(')===false)
				{
					if(preg_match('/^(.*?)(?i:\s+as\s+|\s+)(.*)$/',$column,$matches))
						$columns[$i]=$this->_dialect->quoteColumnName($matches[1]).' AS '.$this->_dialect->quoteColumnName($matches[2]);
					else
						$columns[$i]=$this->_dialect->quoteColumnName($column);
				}
			}
		...
		else
		{
			...
			foreach($tables as $i=>$table)
			{
				if(strpos($table,'(')===false)
				{
					...
					$tables[$i]=$this->_dialect->quoteTableName($matches[1]).' '.$this->_dialect->quoteTableName($matches[2]);
					else
						$tables[$i]=$this->_dialect->quoteTableName($table);
				}
			}
			...
			foreach($columns as $i=>$column)
			{
				...
				elseif(strpos($column,'(')===false)
				{
					if(preg_match('/^(.*?)\s+(asc|desc)$/i',$column,$matches))
						$columns[$i]=$this->_dialect->quoteColumnName($matches[1]).' '.strtoupper($matches[2]);
					else
						$columns[$i]=$this->_dialect->quoteColumnName($column);
				}
			}
	 ...
	 * @throws Exception if unknown operator is used
	...
	private function processConditions($conditions)
	{
		...
		if(strpos($column,'(')===false)
			$column=$this->_dialect->quoteColumnName($column);
		...
		if($operator==='IN' || $operator==='NOT IN')
		{
			...
			{
				$paramName = static::PARAM_PREFIX.$this->_paramCounter;
				$values[$i]=$paramName;
		...
		throw new Exception(\Yii::t('yii', 'Unknown operator "{operator}".', array('{operator}'=>$operator)));
	}
	...
	private function joinInternal($type, $table, $conditions='', $params=array())
	{
		...
		{
			if(preg_match('/^(.*?)(?i:\s+as\s+|\s+)(.*)$/',$table,$matches))  // with alias
			$table=$this->_dialect->quoteTableName($matches[1]).' '.$this->_dialect->quoteTableName($matches[2]);
			else
				$table=$this->_dialect->quoteTableName($table);
		}
