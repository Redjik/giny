...
class Connection implements ConnectionInterface
{
	...
	protected function open()
	{
		if($this->_pdo===null)
		{
			...
			if(empty($connectionString))
				throw new Exception(\Yii::t('giny','Connection.connectionString cannot be empty.'));
			try
			{
				\Yii::trace('Opening DB connection','giny.db.Connection');
			...
			catch(\PDOException $e)
			{
				if(YII_DEBUG)
				{
					throw new Exception(\Yii::t('giny','Connection failed to open the DB connection: '.
										   $e->getMessage(),(int)$e->getCode(),$e->errorInfo));
				}
				else
				{
					\Yii::log($e->getMessage(),\CLogger::LEVEL_ERROR,'exception.CDbException');
					throw new Exception(\Yii::t('giny','Connection failed to open the DB connection.',(int)$e->getCode(),$e->errorInfo));
				}
			}
		}
	}
	...
	{
		\Yii::trace('Closing DB connection','giny.db.Connection');
		$this->_pdo=null;
		$this->_active=false;
		$this->_schema=null;
	}
	...
	protected function createPdoInstance()
	{
		...
		if(!class_exists($pdoClass))
			throw new \PDOException(\Yii::t('giny','Connection is unable to find PDO class "{className}". Make sure PDO is installed correctly.',
	...
	public function getStats()
	{
		...
		$timings=$logger->getProfilingResults(null,'giny.db.Query.query');
		$count=count($timings);
		$time=array_sum($timings);
		$timings=$logger->getProfilingResults(null,'giny.db.Query.execute');
		$count+=count($timings);
		$time+=array_sum($timings);
